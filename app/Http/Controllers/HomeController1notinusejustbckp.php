...
class HomeController extends Controller
{
    ...
    //     }
    //     $email = $request->email;
    //     $phoneNumber = $request->countryCode ? $request->countryCode . $request->number : $request->number;
    //     if (!$isOtpStep) {
    //         if (SeoCompany::where('email', $email)->exists()) {
    //             return response()->json(['success' => false, 'message' => 'This email already exists.'], 422);
    //         }

    //         $fullPhone = '+' . ltrim($request->countryCode, '+') . $request->number;
    //         $accessKey = 'b3efc851c52e731273540429403e9794';
    //         $verifyUrl = "https://apilayer.net/api/validate?access_key=$accessKey&number=" . urlencode($fullPhone);
    //         $response = Http::get($verifyUrl);
    //         $phoneData = $response->json();
    //         $isValidPhone = $phoneData['valid'] ?? false;
    //         $isMobile = ($phoneData['line_type'] ?? '') === 'mobile';
    //         if (!$isValidPhone || !$isMobile) {
    //             return response()->json([
    //                 'success' => false,
    ...
    //             return response()->json(['success' => true, 'message' => 'Form submitted successfully!', 'refresh' => true]);
    //         } else {
    //             return response()->json(['success' => false, 'message' => 'Invalid or expired OTP.']);
    //         }
    //     }
    ...
    private function validatePhoneNumber($countryCode, $phoneNumber)
    {
        try {
            ...
            $accessKey = '4ff091d83e999f191befe83856b18cd4';
