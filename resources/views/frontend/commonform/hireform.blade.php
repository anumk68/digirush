<!-- <form  action="{{ route('hireform.store') }}" method="POST" id="hireForm"> -->
<form  action="{{ route('hire.submit') }}" method="POST" id="hireForm">
    @csrf
    ...
    <div class="row">
        <div class="col-md-6">
            ...
            <input type="text" id="fname" name="fname" oninput="this.value = this.value.replace(/[^a-zA-Z\s]/g, '');" placeholder="First Name" value="{{ old('fname') }}">
        ...
        <div class="col-md-6">
            ...
            <input type="text" id="lname" name="lname" oninput="this.value = this.value.replace(/[^a-zA-Z\s]/g, '');" placeholder="Last name" value="{{ old('lname') }}">
            <div class="text-danger"></div>
            @error('lname')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        ...
        <div class="col-md-6">
            ...
            <input type="tel" id="phone_no" name="number" oninput="this.value = this.value.replace(/[^0-9]/g, '');" placeholder="Phone No" value="{{ old('phone_no') }}" pattern="^\d{10}$" maxlength="10">
            <div class="text-danger"></div>
            @error('phone_no')
                <div class="text-danger">{{ $message }}</div>
            @enderror
